...
def _spectral_high_frequency_relative_l2(
    pred: torch.Tensor,
    target: torch.Tensor,
    cutoff: float = 0.25,
    eps: float = 1.0e-8,
) -> torch.Tensor:
    """Relative L2 error restricted to high Fourier frequencies."""
    ...
    fx = torch.fft.rfftfreq((width_half - 1) * 2, device=pred.device).abs().view(1, 1, 1, width_half)
    mask = (fx + fy) >= float(cutoff)
    numerator = torch.linalg.vector_norm((pred_fft - target_fft) * mask)
    denominator = torch.linalg.vector_norm(target_fft * mask).clamp_min(float(eps))
    return numerator / denominator


def frequency_weighted_mse(pred, target, high_frequency_weight=0.02):
    """Relative L2 plus gradient and high-frequency Fourier loss.

    The benchmark harness imports this historical helper name as the custom
    loss entry point.  The third argument is interpreted as a small spectral
    high-frequency loss weight for this HF follow-up candidate.
    """

    gradient_weight = 0.05
    spectral_weight = 0.02 if high_frequency_weight is None else float(high_frequency_weight)
    data_loss = _relative_l2_loss(pred, target)
    gradient_loss = _finite_difference_gradient_relative_l2(pred, target)
    spectral_loss = _spectral_high_frequency_relative_l2(pred, target)
    return data_loss + gradient_weight * gradient_loss + spectral_weight * spectral_loss
